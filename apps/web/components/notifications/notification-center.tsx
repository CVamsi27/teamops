...
import { Bell, Check, Clock, Users, CheckSquare, FolderOpen } from "lucide-react";
import { Popover, PopoverContent, PopoverTrigger } from "@workspace/ui/components/popover";
import { Separator } from "@workspace/ui/components/separator";
import { toast } from "@workspace/ui/components/toast";
import { useNotificationsList } from "@/hooks/useNotificationsList";
import { useApiMutation } from "@/hooks/api/useApiMutation";
import { type Notification, MarkAsReadSchema, MarkAllAsReadSchema } from "@workspace/api";
...
export function NotificationCenter() {
  ...
  const markAllAsRead = useApiMutation<{ success: boolean }, Record<string, never>>(
  ...
  const getNotificationIcon = (type: string) => {
    switch (type) {
      case 'TASK_ASSIGNED': return <CheckSquare className="h-4 w-4" />;
      case 'TASK_UPDATED': return <Clock className="h-4 w-4" />;
      case 'TEAM_INVITE': return <Users className="h-4 w-4" />;
  ...
  const getNotificationColor = (type: string) => {
    switch (type) {
      ...
      case 'TEAM_INVITE': return 'bg-green-500';
              ...
              {notifications.map((notification: Notification) => (
                ...
                >
                  <div className="flex items-start space-x-3">
                    <div className={`rounded-full p-1 ${getNotificationColor(notification.type)}`}>
