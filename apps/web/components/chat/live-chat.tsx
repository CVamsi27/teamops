...
export function LiveChat({ roomId, roomType, roomName }: LiveChatProps) {
  ...
  useEffect(() => {
    ...
    // Load chat history
    socket.emit('get_chat_history', { roomId, roomType: roomType.toUpperCase() });
    socket.on('chat_history', (history: ChatMessage[]) => {
      setMessages(history);
    });

    return () => {
      socket.emit('leave_chat', { roomId, roomType: roomType.toUpperCase(), userId: currentUser.id });
  ...
  const sendMessage = (data: { message: string }) => {
    ...
    const message: Omit<ChatMessage, 'id'> = {
      ...
      userId: currentUser.id,
      userName: currentUser.name || currentUser.email.split('@')[0] || 'Unknown',
  ...
  return (
    <Card className="h-[500px] flex flex-col">
      ...
      <CardContent className="flex-1 flex flex-col gap-3 p-0">
        ...
        <form onSubmit={form.handleSubmit(sendMessage)} className="flex gap-2 p-4 border-t">
          <Input
            ...
            autoComplete="off"
          />
          <Button type="submit" size="sm" disabled={!form.watch("message")?.trim()}>
