...
export function ActivityTimeline({ entityId, entityType, limit = 50 }: ActivityTimelineProps) {
  const endpoint = entityId && entityType 
  ...
  const { data: activities, isLoading, error } = useApiQuery<ActivityEvent[]>(
    queryKey,
    endpoint,
  ...
  const getActivityBadge = (activity: ActivityEvent) => {
    ...
    if (type.includes('project')) return <Badge variant="secondary">Project</Badge>;
    if (type.includes('team') || type.includes('member')) return <Badge variant="default">Team</Badge>;
    if (type === 'comment_added') return <Badge variant="secondary">Comment</Badge>;
              ...
              {displayedActivities.map((activity, index) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {activity.metadata?.comment && typeof activity.metadata.comment === 'string' ? (
